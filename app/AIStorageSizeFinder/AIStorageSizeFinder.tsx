...
const UNITS: Unit[] = [
  { size: '5x5', sqft: 25, cuft: 200, label: 'Closet Sized', bestFor: 'Boxes, seasonal items, small furniture', rooms: 'Closet or less', color: '#E8F4FD', accent: '#2196F3' },
  { size: '5x10', sqft: 50, cuft: 400, label: 'Walk-In Closet', bestFor: 'Studio or dorm contents', rooms: 'Studio apartment', color: '#F3E8FD', accent: '#9C27B0' },
  { size: '5x15', sqft: 75, cuft: 600, label: 'Large Walk-In', bestFor: 'Small apartment contents', rooms: '1-bedroom apartment', color: '#E8FDF3', accent: '#4CAF50' },
  { size: '10x10', sqft: 100, cuft: 800, label: 'Half Garage', bestFor: '1-bedroom apartment', rooms: '2-bedroom apartment', color: '#FDF8E8', accent: '#FF9800' },
  { size: '10x15', sqft: 150, cuft: 1200, label: 'Full Garage', bestFor: '2-bedroom apartment or small house', rooms: '2-3 bedroom home', color: '#FDE8E8', accent: '#F44336' },
  { size: '10x20', sqft: 200, cuft: 1600, label: 'One-Car Garage', bestFor: 'Full house or vehicle storage', rooms: '3-4 bedroom home', color: '#E8EDF8', accent: '#3F51B5' },
  { size: '10x25', sqft: 250, cuft: 2000, label: 'Large Garage', bestFor: 'Large home or commercial use', rooms: '4-5 bedroom home', color: '#F8EDE8', accent: '#795548' },
  { size: '10x30', sqft: 300, cuft: 2400, label: 'Extra Large', bestFor: 'Business inventory or full estate', rooms: 'Large home / commercial', color: '#EEF8E8', accent: '#689F38' },
]

// Reservation target for "Reserve This Size" CTA — stays on this site,
// scrolls user to the location finder so they pick a facility first.
...
// "AI" explanation — rules-based personalized summary using the user's choices.
function buildExplanation(
  ...
  const sit = situationId && sitMap[situationId] ? sitMap[situationId] : 'space'
  const dens = densityId && densMap[densityId] ? densMap[densityId] : ''
...
function UnitVisual({ size, fillPct }: { size: string; fillPct: number }) {
  ...
  return (
    ...
    >
      {/* Floor */}
      <polygon
        points={`${depth},${wallH + depth} ${depth + floorW},${wallH + depth} ${depth + floorW},${wallH + depth + floorH} ${depth},${wallH + depth + floorH}`}
        fill="#D0D0D0"
        stroke="#aaa"
        strokeWidth="1"
      />
      ...
      {/* Left wall */}
      <polygon
        points={`0,0 ${depth},${depth} ${depth},${depth + wallH} 0,${wallH}`}
        fill="#D5D5D5"
        stroke="#aaa"
        strokeWidth="1"
      />
      {/* Roof */}
      <polygon
        points={`0,0 ${depth},${depth} ${depth + floorW},${depth} ${floorW},0`}
        fill="#F0F0F0"
        stroke="#aaa"
        strokeWidth="1"
      />
        ...
        <>
          ...
          {[...Array(Math.min(Math.ceil(filled * 5), 5))].map((_, i) => (
            <rect
              key={i}
              x={depth + 8 + i * 20}
              y={depth + wallH - Math.min(boxH - 8, wallH - 8)}
              width={16}
              height={Math.min(boxH - 8, wallH - 8)}
              fill="#F60001"
              opacity={0.35 + i * 0.06}
              rx="2"
            />
      ...
      {[...Array(total)].map((_, i) => (
        <div
          ...
          style={{
            ...
            height: 7,
            borderRadius: 99,
            background: i === step ? '#F60001' : i < step ? '#1A1A1A' : '#DDD',
            transition: 'all 0.3s',
          }}
        />
      ))}
...
// ─── OPTION GRID ─────────────────────────────────────────────────────────────
type GridOption = { id: string; label: string; icon: string; sub?: string }
      ...
      {options.map((opt) => {
        ...
        return (
          ...
          >
            ...
            {opt.sub && (
              <div style={{ fontSize: 11, color: active ? '#aaa' : '#999', marginTop: 3 }}>
                {opt.sub}
              </div>
            )}
          </button>
        ...
        {ADD_ONS.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <div
                  style={{ fontSize: 12, fontWeight: 600, color: on ? '#fff' : '#1A1A1A' }}
                >
                  {item.label}
                </div>
                <div style={{ fontSize: 10, color: on ? '#aaa' : '#999' }}>
                  +{item.cuft} cu ft
                </div>
              </div>
            </button>
      ...
      >
        ...
        <div style={{ marginBottom: 6 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span
              style={{ fontSize: 12, color: '#888', fontFamily: "'DM Sans', sans-serif" }}
            >
              Confidence meter
            </span>
            <span
        ...
        {[rec, bigger].filter((u): u is Unit => Boolean(u)).map((unit, i) => (
          <div
            ...
            style={{
              flex: 1,
              background: '#fff',
              border: i === 0 ? '2px solid #F60001' : '2px solid #E8E8E8',
              borderRadius: 16,
              padding: '16px 14px',
              position: 'relative',
            }}
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 10 }}>
              <UnitVisual
                size={unit.size}
                fillPct={i === 0 ? conf : getConfidence(totalCuft, unit)}
              />
            </div>
            <div
              style={{
...
export default function AIStorageSizeFinder() {
  ...
  // Lock the body scroll while this overlay is mounted so the page chrome
  // behind it never peeks through during scroll.
  useEffect(() => {
  ...
  const totalCuft = (() => {
    ...
    const extras = ADD_ONS.filter((a) => addOns.includes(a.id)).reduce(
      (s, a) => s + a.cuft,
      0,
    )
    ...
    >
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh' }}>
        ...
        >
          ...
          >
            MODERN STORAGE
            <sup style={{ fontSize: '0.55em', fontWeight: 'bold', position: 'relative', top: '-0.6em', marginLeft: 2 }}>
              ®
            </sup>
          </div>
          <div
          ...
          >
            {step < 2 ? 'AI Storage Size Finder' : 'Your Best Fit'}
          </div>
          <div style={{ fontSize: 12, color: '#777', marginTop: 3 }}>
            {step < 2
              ? 'Find the right unit size in under 30 seconds.'
            ...
            <div>
              ...
              <button
                ...
                style={{
                  marginTop: 20,
                  width: '100%',
                  background: situation ? '#F60001' : '#EEE',
                  border: 'none',
                  borderRadius: 14,
                  padding: '18px',
                  color: situation ? '#fff' : '#bbb',
                  fontFamily: "'Bebas Neue', cursive",
                  fontSize: 20,
                  letterSpacing: '0.08em',
                  cursor: situation ? 'pointer' : 'default',
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
                ...
                <button
                  ...
                  style={{
                    flex: 2,
                    background: density ? '#F60001' : '#EEE',
                    border: 'none',
                    borderRadius: 14,
                    padding: '14px',
                    color: density ? '#fff' : '#bbb',
                    fontFamily: "'Bebas Neue', cursive",
                    fontSize: 20,
                    letterSpacing: '0.08em',
                    cursor: density ? 'pointer' : 'default',
                  }}
                >
                  Find My Size →
                </button>
